...
function viewInvoice(invoiceId) {
    ...
    if (companySettings && companySettings.logo) {
        logoElement.src = companySettings.logo;
    } else {
        logoElement.src = 'assets/default-logo.png';
    }
    ...
    try {
        ...
        setTimeout(() => {
            try {
                ...
                const success = exportToPDF('invoice-preview', `Invoice_${invoiceId}.pdf`);
                
                // If primary method fails, use fallback
                if (!success) {
                    const fallbackSuccess = downloadInvoiceAsPDF(invoiceId);
                    if (fallbackSuccess) {
                        showNotification('PDF downloaded successfully', 'success');
                    } else {
                        // If both methods fail, try the simple test PDF
                        const testSuccess = generateSimplePDF(invoiceId);
                        if (testSuccess) {
                            showNotification('Generated a test PDF. Please check your downloads.', 'warning');
                        } else {
